...
public class CoordinateSys {
    ...
    public CoordinateSys(FTCRobot robot, LinearOpMode curOpMode, double x, double y, double angle){
        ...
        this.prevPose[2] = this.pose[2] = startingAngle = angle;
...
//        double distance = elapsedEncoderCounts.getDistanceTravelledInInches() * 2;
        double distanceL = elapsedEncoderCounts.getDistanceTravelledInInchesLeft();
        double distanceR = elapsedEncoderCounts.getDistanceTravelledInInchesRight();
        double distance = (distanceL + distanceR) / 2;

//        pose[2] = prevPose[2] + ((distanceR - distanceL) / robot.distanceBetweenWheels);
        pose[2] = robot.navigation.gyro.getYaw_cc0_180scale();
        pose[0] = prevPose[0] + distance * Math.cos(Math.toRadians(prevPose[2]));
        pose[1] = prevPose[1] + distance * Math.sin(Math.toRadians(prevPose[2]));
